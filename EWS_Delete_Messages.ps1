...
<#
     .SYNOPSIS
        The PowerShell script which can be used to delete the messages under the specific 
        conditions for a mailbox (Exchange online or Exchange onprem). Support to run the script in PowerShell 5 only. 
    .DESCRIPTION
        The PowerShell script which can be used to delete the messages under the specific 
        conditions for a mailbox (Exchange online or Exchange onprem). Support to run the script in PowerShell 5 only. 
    .PARAMETER  
        Mailbox: the user mailbox to detele the messages. Mandatory
    ...
    .EXAMPLE
        ...
        $messageid= "<SI2P153MB0458C8D533606DCB334A2211D2099@SI2P153MB0458.APCP153.PROD.OUTLOOK.COM>";
        .\DeleteMessages.ps1 -Mailbox $mailbox -StartDateTime $startDateTime -EndDateTime $endDateTime -SubjectString $SubjectString -Sender $Sender -MessageId $MessageId -IsConfirm -ExchangeOnline;
    ...
    .EXAMPLE
        ...
        $SubjectString = "test";
        $Sender = 'test@contoso.com';
        .\DeleteMessages.ps1 -Mailbox $mailbox -StartDateTime $startDateTime -EndDateTime $endDateTime -SubjectString $SubjectString -Sender $Sender -IsConfirm;        
#>
...
Function DeleteMessages {
    ...
    if ($PSCmdlet.ShouldProcess("some found messages", "DeleteMessages")) {
        $idsList = $objList -as 'System.Collections.Generic.List[System.Collections.Generic.List[Microsoft.Exchange.WebServices.Data.ItemId]]';
        if ($null -eq $idsList) {
            Write-Host "the Ids List is null. Exit";
            exit;
        }
        [Microsoft.Exchange.WebServices.Data.SendCancellationsMode]$sendCancellationsMode = [Microsoft.Exchange.WebServices.Data.SendCancellationsMode]::SendToNone;
        [Microsoft.Exchange.WebServices.Data.AffectedTaskOccurrence]$affectedTaskOccurrences = [Microsoft.Exchange.WebServices.Data.AffectedTaskOccurrence]::SpecifiedOccurrenceOnly;
        [Microsoft.Exchange.WebServices.Data.DeleteMode]$DelMode = [Microsoft.Exchange.WebServices.Data.DeleteMode]$DeleteMode;
        ...
        if ($idsList.Count -gt 0) {
            $idsList | ForEach-Object {
                ...
                if ($ids.Count -gt 0) {
                    $exchangeWebService.DeleteItems($ids, $DelMode, $sendCancellationsMode, $affectedTaskOccurrences);
                }
...
if ($idsList.Count -gt 0) {
    if ($IsConfirm) {
        DeleteMessages -exchangeWebService $service -objList $idsList -DeleteMode $DeleteMode -Confirm;
    }
    else {
        DeleteMessages -exchangeWebService $service -objList $idsList -DeleteMode $DeleteMode;
    }
}